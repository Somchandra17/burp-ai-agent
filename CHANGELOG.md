...
- **Burp ScanCheck Traffic Logging**:
  - The Burp-native `AiScanCheck` path now emits AI Logger entries for baseline and attack requests, including phase, vuln class, insertion point, trace ID, and payload metadata.
  - Requests generated by `AiScanCheck` are now also persisted into Burp Site Map, aligning Burp Scanner integration with the standalone active scanner visibility model.

### Changed

- **Safer Active Request Mutation**:
  - Active scanner payload injection now prefers structured Montoya parameter updates for URL, body, cookie, JSON, and XML mutation paths instead of rebuilding requests from raw URL string replacement.
  - Fallback mutation logic now updates query strings, form bodies, cookies, and path segments more deterministically when structured mutation is unavailable.
- **API Version Bypass Noise Reduction**:
  - Tightened active confirmation logic for `API_VERSION_BYPASS` to require stronger API-specific signals and successful API-like responses.
  - Added documentation/changelog suppression patterns and regression tests to reduce false positives from help pages and release notes.
